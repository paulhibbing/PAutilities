...
This dataset has energy expenditure values that we can use to look at bouts of
moderate-to-vigorous physical activity. Before we get there, though, we need to
do a little bit of pre-processing as follows:
...
* **target_buffer = 30** Here we specify how our data should be
  ...
  stratified/partitioned anytime we see 30 consecutive minutes of `other`
...
* **start_index** The starting point of the bout (i.e., `intensity[654]` for
  the first bout in this example)
* **end_index** The ending point of the bout (i.e., `intensity[654]` for
  the first bout in this example)
...
In some cases, you may wish to look only at bouts that last a certain amount of time. Let's restrict to only bouts lasting at least 10 minutes (recalling again that `length` conveniently equates to `minutes` because our input was minute-by-minute data).

```{r filter, cache=TRUE}

mvpa_bouts <- mvpa_bouts[mvpa_bouts$length_value >= 10, ]
head(mvpa_bouts)
...
This should give you a broad sense of how to use the cluster-based bout identification method and what else you can do with it. Feel free to post an issue on the [GitHub page](https://github.com/paulhibbing/PAutilities) if any of the above gives you trouble. Good luck!
